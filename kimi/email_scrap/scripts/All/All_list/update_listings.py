...
"""
================================================================================
FCC ROBOCALL LISTINGS AUTO-UPDATER (ADAPTIVE CONCURRENCY & BULK POST)
================================================================================
This script scrapes listings pages with dynamically adjusting parallel workers 
hitting ServiceNow's JSON widgets loading routers cleanly.

LOGIC:
1. **Adaptive Scaling**: Uses asyncio.Semaphore with dynamic Release adjusting tokens.
2. **Bulk DB inserts**: Buffers Conflict triggers upsert commits efficiently.
...
class AdaptiveScraper:
    ...
    async def fetch_page(self, page_num: int):
        """Worker executing page fetches conforming with Semaphore triggers."""
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                if not rows:
                    ...
                    for r in rows:
                        ...
                        if frn and frn in self.recent_ids:
                            logging.info(f"🚨 Found overlapping sync point FRN='{frn}'. Triggering Early Stop.")
                ...
                logging.warning(f"Page {page_num} HTTP Errors: {resp.status_code}")
                self.stats["errors"] += 1
                # Retry increment fallback frames adding back to queue
                await self.queue.put(page_num)

        except Exception as e:
            logging.error(f"Error page {page_num}: {e}")
            self.stats["errors"] += 1
            if not self.stop_scraping.is_set():
                await self.queue.put(page_num)

        finally:
            # 📈 Adaptive Concurrency scaling handler inside release frames
            if self.concurrency > self.target_concurrency:
                 self.concurrency -= 1 # Consume token directly reducing limit pool
            else:
                 self.sem.release()
    ...
    async def controller_loop(self):
        ...
        while not self.stop_scraping.is_set():
            ...
            if avg_time < 1.8 and error_rate < 0.05 and self.target_concurrency < MAX_CONCURRENCY:
                self.target_concurrency += 1
                self.concurrency += 1
                self.sem.release() # Release extra token
                logging.info(f"🚀 Scaling UP Concurrency -> {self.target_concurrency}")

            # Scale Down: Slows trigger drops limits
            elif (avg_time > 3.5 or error_rate > 0.10) and self.target_concurrency > MIN_CONCURRENCY:
                self.target_concurrency -= 1
                logging.info(f"🐢 Scaling DOWN Concurrency -> {self.target_concurrency}")

    async def run(self):
        """Main Orchestrator seeding tasks continuous."""
        ...
        while not self.queue.empty() and not self.stop_scraping.is_set():
             page = await self.queue.get()
             asyncio.create_task(self.fetch_page(page))
             await asyncio.sleep(0.05) # Jitter

        # Wait remaining executing frames
        while self.sem._value < self.concurrency and not self.stop_scraping.is_set():
...
async def main():
    ...
    if not token:
        logging.error("Failed to extract session token.")
        return

    recent_ids = await fetch_existing_ids()
    logging.info(f"Loaded {len(recent_ids)} historical IDs for Early-Stop caching.")
